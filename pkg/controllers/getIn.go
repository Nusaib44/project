...
func Signup(g *gin.Context) {
	...
	if phone.ID > 0 {
		println("login db check phone number", phone.PhoneNumber)
	...
	g.JSON(http.StatusOK, gin.H{
		"message": "signed in surcessfully",
	})
	var userid int
...
func Login(g *gin.Context) {
	...
	// binding json to go response writter
	if g.Bind(&body) != nil {
		g.JSON(400, gin.H{"error": "failed to bind and load"})
		return
	}
	// checking the user
	var user models.Userdata
	...
	if user.ID == 0 {
		g.JSON(400, gin.H{
			"error": "invalid email check email or try signup",
		})
		return
	}
	...
	// check block
	if !user.Status {
		g.JSON(400, gin.H{"error": "u are blocked"})
		return
	}
...
func admintoken(email string, g *gin.Context) {
	...
	// sending token
	g.SetSameSite(http.SameSiteLaxMode)
	g.SetCookie("admincoookie", tokenString, 3600*24*30, "", "", false, true)
